...
def df_parsed(df):
    ...
    """
    return pd.to_datetime(df['event_time'], format='%Y-%m-%d %H:%M:%S %Z')
...
def avg_price_cat(df, category):
    ...
    avg_prices = df_purchases.loc[df_purchases['category_id'] == category].groupby('brand').mean()['price']
...
def plot_pareto(df, step=10, color='darkorange'):
    ...
    paretos_df = pd.DataFrame(index=x, data=paretos).rename(columns={0: 'Pareto Behaviour'})

    plot_bar(to_plot=paretos_df,
             title='Pareto principle w.r.t. percentage of users - step of {}'.format(step),
