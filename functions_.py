...
def avg_price_cat(df_paths, category):
    ...
    for frame in df:
        ...
        if not frame.empty:
            ...
            frame = frame.loc[results['category'] == category].groupby('brand', sort=False).apply(f)
    ...
    # There are too many brands for a single category, change the xticks
    brands = results.index
